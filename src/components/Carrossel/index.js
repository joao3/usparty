...
const Carrossel = (props) => {
  ...
  const handleLeftArrow = () => {
    const newIndex = currentIndex === 0 ? props.banners.length - 1 : currentIndex - 1;
    setCurrentIndex(newIndex);
  }

  const handleRightArrow = () => {
    const newIndex = currentIndex === props.banners.length - 1 ? 0 : currentIndex + 1;
    setCurrentIndex(newIndex);
  }
      ...
      <div style={{
        ...
        {props.banners.map((slide, slideIndex) => {
          return slideIndex === currentIndex ?
            <div onClick={() => goToSlide(slideIndex)} key={slideIndex} style={{
              color: '#820AD1',
              margin: '0 3px',
              cursor: 'pointer',
              fontSize: '20px'
