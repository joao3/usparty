...
const Carrinho = (props) => {
  ...
  const removerEvento = (evento) => {
    ...
    if (carrinhoCopia) {
      const index = carrinhoCopia.itens.indexOf(
        carrinhoCopia.itens.find((e) => e.evento.path == evento.path)
      );
      if (index > -1) {
        carrinhoCopia.total -=
          carrinho.itens[index].quantidade * carrinho.itens[index].evento.valor;
        ...
        {carrinho && carrinho.itens.length > 0 ? (
          <>
            ...
            <div className="bottom">
              <div className="total">
                Total: R${" "}
                {carrinho && carrinho.total.toFixed(2).replace(".", ",")}
              </div>
